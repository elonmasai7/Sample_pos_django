...
class folioAccountsPanel ( wx.Panel ):
	...
	def populateTable (self):
		...
		qry = 'SELECT gl.id, gl.dateTime, hoa.description, gl.headOfAc, gl.transactionType, gl.chequeNo, gl.Debit, gl.Credit, ( SELECT %s FROM generalLedger WHERE id <= gl.id and headOfAc = "%s") AS Balance FROM generalLedger gl, headOfAccounts hoa where gl.headOfAc = hoa.id AND gl.dateTime BETWEEN "%s" AND "%s" AND hoa.description = "%s" LIMIT 500' % ( computation, self.folios[self.selectedFolio], self.m_startDate.GetValue().Format("%F") + " 00:00:00", self.m_endDate.GetValue().Format("%F") + " 23:59:59", self.selectedFolio)
